...
Open this file and replace the "Confirmed TTS contract" block below with the actual values. Every later task refers back to these constants.

```markdown
<!-- Confirmed TTS contract (fill in after probe) -->
...
git commit -m "docs(talk-mode): pin OpenRouter TTS endpoint after probe"
```
